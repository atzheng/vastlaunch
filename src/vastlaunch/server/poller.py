...
def log_job(job_id: str, msg: str) -> None:
    """Log a message and persist it to the job's poller_log in the DB."""
    log(f"{job_id}: {msg}")
    try:
        ts = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
        state.append_poller_log(job_id, f"[{ts}] {msg}\n")
    except Exception:
        pass
...
def _advance(job: dict) -> None:
    status = job["status"]
    job_id = job["job_id"]
    log_job(job_id, f"status={status}")
    if status == "queued":
        _do_launch(job)
...
def _do_launch(job: dict) -> None:
    ...
    try:
        ...
        log_job(job_id, f"no offer found — {e}")
    ...
    try:
        ...
        log_job(job_id, f"create_instance failed — {e}")
        state.update_by_job_id(job_id, status="failed")
        return
...
def _check_connecting(job: dict) -> None:
    ...
    if since > _CONNECTING_TIMEOUT:
        log_job(job_id, f"SSH connect timeout after {int(since)}s")
        _destroy_and_fail(job)
        return
    ...
    try:
        ...
        log_job(job_id, f"SSH probe timed out (key={_SSH_KEY})")
        return
    if proc.returncode != 0 or "ok" not in proc.stdout:
        log_job(job_id, f"SSH probe failed rc={proc.returncode} stderr={proc.stderr.strip()!r} (key={_SSH_KEY})")
    ...
    if workdir_key:
        try:
            ...
            log_job(job_id, f"workdir download/rsync failed — {e}")
            _destroy_and_fail(job)
            return
    else:
        ...
        if workdir and workdir.exists():
            try:
                ...
                log_job(job_id, f"rsync failed — {e}")
                _destroy_and_fail(job)
                return
    ...
    try:
        ...
        log_job(job_id, f"failed to start job — {e}")
        _destroy_and_fail(job)
        return

    state.update_by_job_id(job_id, status="running")
    log_job(job_id, "job started in tmux")
...
def _check_running(job: dict) -> None:
        ...
        if instance_id:
            try:
                ...
                if actual in _TERMINAL_ERROR_STATUSES:
                    log_job(job_id, f"instance went {actual} without exit code")
                    state.update_by_job_id(job_id, status="failed")
    ...
    try:
        ...
        log_job(job_id, f"could not capture logs — {e}")

    state.update_by_job_id(job_id, status=new_status, exit_code=code, logs=full_log)
    log_job(job_id, f"finished — status={new_status} exit_code={code}")
    ...
    if auto_destroy and instance_id:
        try:
            ...
            log_job(job_id, f"instance {instance_id} destroyed")
        except Exception as e:
            log_job(job_id, f"destroy failed — {e}")
...
def _load_config(job: dict) -> config.Job | None:
    ...
    if not config_yaml:
        log_job(job["job_id"], "no config_yaml, marking failed")
        state.update_by_job_id(job["job_id"], status="failed")
        return None
    try:
        return config.load_from_string(config_yaml)
    except Exception as e:
        log_job(job["job_id"], f"failed to parse config — {e}")
        state.update_by_job_id(job["job_id"], status="failed")
        return None
